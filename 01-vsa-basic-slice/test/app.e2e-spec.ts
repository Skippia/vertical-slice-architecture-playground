...
describe('VSA Basic Slice E2E', () => {
  ...
  // ──────────────────────────────────────────────
  // Cross-slice: EventBus — stock reduction
  // ──────────────────────────────────────────────
  describe('Cross-slice — stock reduction via EventBus', () => {
    it('should reduce product stock after checkout', async () => {
      ...
      // Wait for the EventBus handler to process
    ...
    it('should reduce stock correctly after multiple checkouts', async () => {
      const productRes = await request(app.getHttpServer())
        .post('/products')
        .send({ name: 'MultiCheckout', price: 10.0, stock: 100 })
        .expect(201);
      const pid = productRes.body.id;

      // First checkout: 30
      await request(app.getHttpServer())
        .post('/orders/checkout')
        .send({ userId, items: [{ productId: pid, quantity: 30 }] })
        .expect(201);
      ...
      await request(app.getHttpServer())
        .post('/orders/checkout')
        .send({ userId, items: [{ productId: pid, quantity: 25 }] })
        .expect(201);
      ...
      expect(res.body.stock).toBe(45); // 100 - 30 - 25
    });
  });
  ...
  describe('Full E2E flow — create product, user, checkout, verify', () => {
    it('should execute complete checkout flow', async () => {
      ...
      const product = await request(app.getHttpServer())
        .post('/products')
        .send({ name: 'E2E Flow Product', price: 100, stock: 5 })
        .expect(201);
